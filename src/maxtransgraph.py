...
###############################
#
# Max-Trans-Graph method for taxonomy induction from the paper:
...
def mtg(entities, relfile, reltype='hypernym', minscore=1.5, lmda=1.0,
        noise=0., wn=False):
    '''
...
def solve_exact(g_i):
    ...
    weights = dict([((i,j), g_i[i].get(j,{}).get('weight', 0.)) for i,j in possedges])
    ...
    while True:
        ...
        violated_constraints = get_violated_constraints(x)
        if len(violated_constraints) == 0:
            break
        constr = m.addConstrs((x[i,j]+x[j,k]-x[i,k] <= 1 for i,j,k in violated_constraints), 
                              name='c')
...
def get_violated_constraints(x):
    '''
    '''
    viol = set([])
...
if __name__=="__main__":
    parser = argparse.ArgumentParser(description='Max-Trans-Graph with a single relationtype.')
    ...
    pruned, stats = mtg(entities, relationfile, reltype=relationtype, 
                        minscore=minscore, lmda=p_lambda, noise=noise, wn=wn)
    ...
    util.write_log(pruned, stats, fname=os.path.join(logdir, 'mtg.terms-%s.rel-%s.minsc%0.1f.lambda-%0.1f' % (bn, relationtype, minscore, p_lambda)))